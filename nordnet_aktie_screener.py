...
def test_write():
    exclude_list = []
    my_stocks = ["ALMB", "FFF"]
    my_stocks_json = {"my_stocks" : my_stocks}

    exclude_stocks = ["TOP", "FFF"]
    exclude_stocks_json = {"exclude_stocks" : exclude_stocks}

    all_stocks_json = {"all_stocks": [my_stocks_json, exclude_stocks_json]}
    json_object = json.dumps(all_stocks_json, indent=4)
    with open("exclude_list.json", "w") as outfile:
